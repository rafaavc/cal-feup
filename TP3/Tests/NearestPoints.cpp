...
Result nearestPoints_BF(vector<Point> &vp) {
    //cout << "\n------------ VP SIZE: " << vp.size() << endl;
    if (vp.size() == 0) return Result();
    Result res = Result();
	for (auto it = vp.begin(); it != vp.end(); it++) {
	    for (auto it1 = it+1; it1 != vp.end(); it1++) {
            if (*it == *it1) return Result(0, *it, *it1);
            double dist = (*it).distance(*it1);
            if (dist < res.dmin) {
                res = Result(dist, *it, *it1);
            }
        }
	}
	return res;
